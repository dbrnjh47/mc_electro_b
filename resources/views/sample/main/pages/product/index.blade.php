    ...
    <section class="product product__container">
            ...
            <div class="app__title">
                <div class="app__title_wrapper">
                    <h2 class="app__title_text">{{ $product->name }}
                        @if ($product->uuid)
                            <span class="copy_button" data-copy-text="{{ $product->uuid }}">#{{ $product->uuid }}</span>
                        @endif
                        @if ($product->article)
                            <span class="copy_button" data-copy-text="{{ $product->article }}">#{{ $product->article }}</span>
                        @endif
                    </h2>
        ...
        <div class="product__wrapper">
            <section id="sticky_article" class="product_info">
                            ...
                            @if ($product->medias->isEmpty())
                                ...
                                <div class="swiper-slide">
                                    <div class="product_basic_slider__slide">
                                        <img src="{{ $defult_media->path }}" alt="defult" loading="lazy"
                                            decoding="async">
                                    </div>
                                </div>
                            @else
                                @foreach ($product->medias as $media)
                                    <div class="swiper-slide">
                                        <div class="product_basic_slider__slide">
                                            <img src="{{ $media->path }}" alt="{{ $product->name }}" loading="lazy"
                                                decoding="async">
                ...
                @if (
                    $product->desc ||
                        !$product->propertySections->isEmpty() ||
                    ...
                    <section class="product_menu_blocks product_menu_blocks__container">
                        @if ($product->desc)
                            <div @if (!$is_first_block) class="activ" @endif data-block="description">Описание
                            </div>
                            @php $is_first_block = 1; @endphp
                        @endif

                        @if (!$product->propertySections->isEmpty())
                            <div @if (!$is_first_block) class="activ" @endif data-block="characteristics">
                                Характеристики</div>
                            @php $is_first_block = 1; @endphp
                        @endif

                        @if (!$product->reviews->isEmpty())
                            <div @if (!$is_first_block) class="activ" @endif data-block="reviews">Отзывы
                                <span>({{ $product->reviews_count }})</span></div>
                            @php $is_first_block = 1; @endphp
                        @endif
                        ...
                        @if (!$product->documents->isEmpty())
                            <div @if (!$is_first_block) class="activ" @endif data-block="documentations">
                                Документация <span>({{ $product->documents->count() }})</span></div>
                            @php $is_first_block = 1; @endphp
                        @endif
                    </section>
                    ...
                    <div class="product_menu_block product_menu_block__container">
                        ...
                        @if ($product->desc)
                            <section class="product_menu_block__description @if (!$is_first_block) activ @endif"
                        ...
                        @if (!$product->propertySections->isEmpty())
                            <section
                                class="product_menu_block__characteristics @if (!$is_first_block) activ @endif"
                        ...
                        @if (!$product->reviews->isEmpty())
                            <section class="product_menu_block__reviews @if (!$is_first_block) activ @endif"
                                data-block="reviews">
                                ...
                                <div class="product_menu_block__reviews_rating">
                                    @for ($i = 5; $i > 0; $i--)
                                        @php
                                            $review_statistic = Arr::first($review_statistics, function (
                                                $review_statistic,
                                            ) use ($i) {
                                            ...
                                            if ($review_statistic) {
                                                $procent = round(
                                                    ($review_statistic->count / $product->reviews_count) * 100,
                                                    1,
                                                );
                                            }
                                        @endphp
                                ...
                                <div class="product_menu_block__reviews_sorting">
                                    <div id="select2_product_menu_block__reviews_sort" class="select2_sample_nude">
                                        <select class="select2_custom" name="lang" data-dropdown-position="below"
                                            ...
                                            data-dropdown-parent="#select2_product_menu_block__reviews_sort">
                                ...
                                <div class="product_menu_block__reviews_list">
                                    @foreach ($product->reviews as $review)
                                        <x-sample.main.product.information.review :product_name="$product->name" :review="$review">
                        ...
                        @if (!$product->documents->isEmpty())
                            <section
                                class="product_menu_block__documentations @if (!$is_first_block) activ @endif"
                                data-block="documentations">
                                    ...
                                    <div class="product_menu_block__documentations_list">
                                        ...
                                        @foreach ($product->documents as $document)
                                            <a href="{{ $document->path }}" target="_blank"
                                                alt="{{ $document->title }}"> <svg width="15" height="17"
                                                    viewBox="0 0 15 17" xmlns="http://www.w3.org/2000/svg">
                                                    <path
            ...
            <div id="sticky_aside1">
                ...
                <section class="product_result">
                    <div class="product_result__price_wrapper">
                        <p class="product_result__price">₽
                            {{ \App\Models\Product\Product::getPriceText($product->getLowestPrice()) }}
                            {{-- <span>2000</span> --}}
                        </p>
                        <div class="product_result__top_actions">
                            <button class="btn wishlist_action" data-product-id="{{ $product->id }}"
                                ...
                                <span>{{ $product->wishlist_products_count ? 'В избранном' : 'В избранное' }}</span>
                            </button>

                            <x-sample.main.share url="{{ url()->current() }}"
                                text="{{ $product->name }}"></x-sample.main.share>
                        </div>
                    </div>
                    <div class="product_result_count">
                        @if ($product->point_count || $product->points->isNotEmpty())
                            @if ($product->point_count)
                                <p class="orange">{{ $product->point_count }} шт. на удалённом складе</p>
                            @endif
                            ...
                            @if ($product->points->isNotEmpty())
                                ...
                                <ul class="product_result_count__list">
                                    @foreach ($product->points as $point)
                                        <li @if($point->pivot->count <= 10) class="oreng" @endif>
                                            @if($point->pivot->count <= 10)
                                            ...
                                            @endif
                                            <p>{{ $point->address }}</p> <span>{{ $point->pivot->count }} шт.</span>
                                        </li>
                                    @endforeach
                                </ul>
                            @endif
                    ...
                    <div class="product_result_processing" data-product-id="{{ $product->id }}">
                        ...
                        <div class="product_result_processing__actions">
                            ...
                            <div class="product_result_processing__actions_phone copy_button"
                                data-copy-text="+7 900 000 00 00">
